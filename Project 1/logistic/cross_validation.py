...
MISSING_VALUES_THRESHOLDS = [ 1]   #[0, 0.25,0.5,0.75] subir esto 
CORRELATION_THRESHOLDS    = [0.85]   #[0.75, 0.8, 0.85, 0.9, 0.95, 1.0]
OUTLIERS_THRESHOLDS       = [8]     #[3,4,5,6]
AUGMENTATION_DEGREES      = [2]     #[1,2,3,4,5]

# Hyper parameters for GD, SGD and least squares
REG_LOG_REG_LAMBDA = [1]     # [0, 0.1, 1, 10, 100]
REG_LOG_REG_GAMMA    = [ 0.0000001]   # [ 0.00001, 0.000001, 0.0000001, 0.00000001 ] quitar 1e-7
...
def cross_validation_rlr(y, tx, k, mvt=0.7, ct=0.85, ot=3, ad=2, lamb=10, gamma=0.00001):
    ...
    for i in range(k):
        ...
        # Model train
        w, loss     = reg_logistic_regression(y_train, tx_train, lamb, np.zeros(tx_train.shape[1]), 1000, gamma)
...
def cross_validation_lr(y, tx, k, mvt=0.7, ct=0.85, ot=3, ad=2, gamma=0.00001):
    ...
    for i in range(k):
        ...
        # Model train
        w, loss     = logistic_regression(y_train, tx_train, np.zeros(tx_train.shape[1]), 100, gamma)
